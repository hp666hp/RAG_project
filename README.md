...
```bash
python eval/rag_evaluate.py evaluate --quiet
```
说明：默认运行 hybrid retrieval，主指标按 chunk 排名计算，文档级指标仅作为辅助观察，避免把同一文档的多个 chunk 聚合后造成命中率虚高。
如果要评估本地 CrossEncoder 重排，可运行：

```bash
python eval/rag_evaluate.py evaluate --rerank --quiet
```

重排模式会加载本地模型，速度较慢；首次运行还需要确认 `.env`、向量库和模型路径可用。
...
- 评测脚本当前聚焦检索效果评估，已区分严格 chunk 指标和辅助文档指标，还没有覆盖答案正确性打分、人工标注平台和批量实验管理
- 当前项目以本地开发和面试展示为目标，尚未补充 Docker 化部署、线上观测、权限控制和持续集成

## 下一步优化

- 增加答案级评测，如回答正确性、引用一致性和人工复核闭环
